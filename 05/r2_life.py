...
def cell_value(grid: Game, x: int, y: int) -> Cell:
    if 0 <= x < len(grid) and 0 <= y < len(grid):
        return grid[x][y]
    return 0


def live_neighbour_count(grid: Game, x: int, y: int) -> int:
...
def next_value(grid: Game, x: int, y: int) -> Cell:
    assert x < len(grid) and y < len(grid)
...
def life(grid: Game, count: int) -> Game:
    assert len(grid) > 0
    assert count >= 0
