...
def find_slope(points: List[Point], avg_x: float, avg_y: float) -> Optional[float]:
    dividend: float = 0
    divisor: float = 0
...
def regress_points(points: List[Tuple[int, int]]) -> Optional[Tuple[float, float]]:
...
def residuals_vectors(
    x: List[int], y: List[int], alpha: float, beta: float
) -> List[float]:
...
def residuals_points(points: List[Point], alpha: float, beta: float) -> List[float]:
    res = []
    for i in range(len(points)):
...
def main() -> None:
    ...
    residuals_vectors_res = residuals_vectors(
        [x for x, _ in points], [y for _, y in points], expect_alpha, expect_beta
    )
    residuals_points_res = residuals_points(points, expect_alpha, expect_beta)
